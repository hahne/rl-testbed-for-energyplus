...
class EnergyPlusModel2ZoneDataCenterHVAC_wEconomizer_Temp_Fan(EnergyPlusModel):
    ...
    def setup_spaces(self):
        ...
        self.action_space = spaces.Box(low =   np.array([ lo, lo, flow_lo, flow_lo]),
                                       ...
                                       dtype = np.float32)
        self.observation_space = spaces.Box(low =   np.array([-20.0, -20.0, -20.0,          0.0,          0.0,          0.0, -1.0,-1.0,-1.0,-1.0]),
                                            high =  np.array([ 50.0,  50.0,  50.0, 1000000000.0, 1000000000.0, 1000000000.0,  1.0, 1.0, 1.0, 1.0]),
                                            dtype = np.float32)
    ...
    def format_state(self, raw_state):
        return np.array([raw_state[0], raw_state[1], raw_state[2], raw_state[4], raw_state[5], raw_state[6], 0.0, 0.0, 0.0, 0.0])
